...
class ALA:
    ...
    def adaptive_local_aggregation(self, 
        ...
        rand_ratio = max(0.0, min(1.0, self.rand_percent / 100.0))
        rand_num = max(1, int(round(rand_ratio * n)))   # 최소 1 보장
        idxs = np.random.choice(n, size=rand_num, replace=(rand_num > n))
        subset = [self.train_data[i] for i in idxs]
